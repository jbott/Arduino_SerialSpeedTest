...
public:
    ...
    ///
    ///     void onPacketReceived(uint16_t frame_type const uint8_t* buffer, size_t size);
    ///
    /// where frame_type is the type encoded in the frame, buffer is a pointer
    //  to the incoming buffer array, and size is the / number of bytes in the
    //  incoming buffer.
    typedef void (*PacketHandlerFunction)(uint16_t frame_type, const uint8_t* buffer, size_t size);
...
/// \brief A typedef for the default COBS FrameSerial class.
